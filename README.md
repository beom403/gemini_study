...
### 🧪 Custom Skill: Unit Test Specialist 활용
Gemini CLI의 기능을 특정 도메인에 특화시키는 **'Skill(스킬)'** 시스템을 활용한 예시입니다.

- **전문가 모드 전환**: `activate_skill unit-test-wizard` 명령을 통해 Gemini CLI를 숙련된 테스트 엔지니어 모드로 전환합니다.
- **심층 테스트 설계**: 단순한 성공 케이스(Happy Path)를 넘어, **경계값 분석(Edge Case), 예외 처리(Exception Handling), 비정상 입력**에 대한 테스트 케이스를 자율적으로 도출합니다.
- **프로젝트 맥락 유지**: 프로젝트에서 사용하는 테스트 프레임워크(Pytest, Jest 등)와 Mocking 전략을 스스로 파악하여 기존 코드와 일관된 테스트 코드를 작성합니다.

> **핵심 가치**: "AI가 우리 팀의 테스트 표준을 이해하고, 시니어 수준의 빈틈없는 테스트 세트를 자동으로 구성하여 코드 품질을 보장합니다."
